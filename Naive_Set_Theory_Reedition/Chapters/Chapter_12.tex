...
Property \eqref{eq12.3} is known as the \textbf{principle of mathematical induction.}\index{induction}\index{mathematical induction} We shall now add to this list of properties of $\omega$ two others: 
...
\begin{named}[Recursion theorem.]\index{recursion} If $a$ is an element of a set $X$, and if $f$ is a function from $X$ into $X$, then there exists a function $u$ from $\omega$ into $X$ such that $u(0) = a$ and such that $u(n^{+}) = f(u(n))$ for all $n$ in $\omega$. 
\end{named}
...
The proof of the recursion theorem is complete. An application of the recursion theorem is called \textit{definition by induction}\index{definition by induction}. 
